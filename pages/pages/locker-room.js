...
export default function LockerRoom() {
  ...
  // Initial loads + live refresh for scores/matchups
  useEffect(() => {
  ...
  return (
    <div style={styles.page}>
      ...
      <Section
        ...
        right={loading.countdown ? null : countdown?.targetDate ? <CountdownBadge date={countdown.targetDate} /> : null}
      >
        {loading.countdown ? (
          <div>Loading…</div>
        ) : errors.countdown ? (
      ...
      <Section title="This Week’s Matchups (Live)">
        ...
        ) : !matchups || !Array.isArray(matchups) || matchups.length === 0 ? (
      ...
      {/* Message Board (everyone can see) */}
...
function PollBlock({ poll, loading, error }) {
  ...
  async function submitVote(e) {
    ...
    try {
      // POST to backend
      await fetch(
        (process.env.NEXT_PUBLIC_API_BASE || "http://localhost:4000") + `/polls/vote`,
        {
          method: "POST",
      ...
      setMsg("Failed to submit vote: " + e.message);
    } finally {
      setSubmitting(false);
    }
  }
